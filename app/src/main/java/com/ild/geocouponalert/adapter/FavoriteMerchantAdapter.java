...
public class FavoriteMerchantAdapter extends BaseAdapter  implements Filterable {
	...
	public static ArrayList<String> allAvtiveAlertMerchantId;

    public FavoriteMerchantAdapter(Activity a, List<BusinessMaster> d, ArrayList<String> favoriteMerchantIdActive) {
        activity = a;
        data=d;
		mDisplayedValues = d;
        inflater = (LayoutInflater)activity.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        imageLoader=new ImageLoader(activity.getApplicationContext());
		allAvtiveAlertMerchantId = favoriteMerchantIdActive;
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(vi==null){
            vi = inflater.inflate(R.layout.favorite_merchant_row, null);
		...
		if (allAvtiveAlertMerchantId.contains(business.buss_id)) {
			holder.checkUncheck.setImageResource(R.drawable.star_check);
		}else{
			holder.checkUncheck.setImageResource(R.drawable.star_uncheck);
		}
		...
		holder.chckboxLin.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (allAvtiveAlertMerchantId.contains(merchantid)) {
					checkbox_holder.setImageResource(R.drawable.star_uncheck);
					allAvtiveAlertMerchantId.remove(merchantid);
				}else{
					checkbox_holder.setImageResource(R.drawable.star_check);
					allAvtiveAlertMerchantId.add(merchantid);
				}
			}
		});
